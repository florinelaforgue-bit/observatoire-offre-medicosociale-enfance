# DOSSIER_PUBLICATIONS.md

> **Carnet de valorisation scientifique vivant**

Ce document accompagne le projet jusqu'aux publications scientifiques.
Il est enrichi progressivement à partir des artefacts du projet.

## Règles de fonctionnement

-   Ne jamais réécrire une section validée.
-   Ajouter uniquement des éléments démontrés par les artefacts ou
    explicitement identifiés comme des perspectives.
-   Ne jamais inventer de résultats.
-   Chaque nouvel artefact peut conduire à enrichir une ou plusieurs
    sections.

------------------------------------------------------------------------

# 0. Vision scientifique du projet

*À compléter progressivement.*

# 1. Contributions scientifiques originales

*À compléter progressivement.*

# 2. Contributions méthodologiques

*À compléter progressivement.*

# 3. Stratégie de publication

*À compléter progressivement.*

# 4. Questions scientifiques rendues possibles

*À compléter progressivement.*

# 5. Résultats déjà démontrés

*À compléter progressivement.*

# 6. Résultats restant à démontrer

*À compléter progressivement.*

# 7. Expériences réalisées

*À compléter progressivement.*

# 8. Expériences complémentaires à prévoir

*À compléter progressivement.*

# 9. Figures, tableaux et schémas

*À compléter progressivement.*

# 10. Revues scientifiques potentielles

*À compléter progressivement.*

# 11. Références bibliographiques à rechercher

*Lister uniquement des thèmes bibliographiques à documenter.*

# 12. Collaborations potentielles

*À compléter progressivement.*

# 13. Financements potentiels

*À compléter progressivement.*

# 14. Tableau de bord scientifique

  ------------------------------------------------------------------------
  Contribution   Démontrée   Niveau de preuve   Publication cible   État
  -------------- ----------- ------------------ ------------------- ------
  *(à                                                               
  compléter)*                                                       

  ------------------------------------------------------------------------

# 15. Questions ouvertes

Cette section recense les questions qui sont apparues pendant le
développement mais qui nécessitent des travaux futurs.

Pour chacune :

- contexte ;
- pourquoi la question est importante ;
- état actuel ;
- quelles données seraient nécessaires ;
- publication(s) potentiellement concernée(s).
